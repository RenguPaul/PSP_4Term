    ...
    getHTML(data) {
        return `
            <div class="detail-card">
                ...
                <h3>Редактировать</h3>
                <form id="edit-form">
                    <div class="form-group"><input type="text" id="edit-title" value="${this.escapeHtml(data.title)}" placeholder="Заголовок" required></div>
                    <div class="form-group"><input type="text" id="edit-src" value="${this.escapeHtml(data.src)}" placeholder="URL изображения" required></div>
                    <div class="form-group"><textarea id="edit-text" rows="3" required>${this.escapeHtml(data.text)}</textarea></div>
